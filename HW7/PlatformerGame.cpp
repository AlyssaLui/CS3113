...
void PlatformerGame::Init() {
	...
	effectTexture = loadTexture("effectTextures.png");
	glow = new Entity(effectTexture, 0.0, 53.0 / 512.0, 256.0 / 512.0, 256.0 / 512.0, true);
	word = new Entity(effectTexture, 0.0, 0.0, 304.0 / 512.0, 51.0 / 512.0, true);
	word->setX(0.0);
	word->setY(0.8);
	word->setRotation(90.0);
...
bool PlatformerGame::UpdateandRender() {
	...
	if (keys[SDL_SCANCODE_LEFT]) {
		rocket->setRotation(rocket->getRotation() + 150 * (elapsed));
		...
		GLfloat rotate = (rocket->getRotation() * PI) / 180.0f;

		rocket->setAccelY(sin(rotate));
		rocket->setAccelX(cos(rotate));
	}
	else {
	...
	if (animationTime > 0.5) {
		animationValue = mapValue(animationTime, 1.0, 0.5, 0.0, 1.0);
	}
	else {
		animationValue = mapValue(animationTime, 0.0, 0.5, 0.0, 1.0);
	}

	animationTime2 += elapsed;
	if (animationTime2 > 2.0) {
		animationValue2 = mapValue(animationTime2, 4.0, 2.0, 0.0, 1.0);
	}
	else {
		animationValue2 = mapValue(animationTime2, 0.0, 2.0, 0.0, 1.0);
	}
...
void PlatformerGame::FixedUpdate() {
	...
	rocket->FixedUpdate();
	glow->setX(rocket->getX());
	glow->setY(rocket->getY());
	...
	word->setX(easeIn(1.0, 0.0, animationValue2));
...
void PlatformerGame::Render() {
	...
	glTranslatef(0.0, sin(animationValue * screenShakeSpeed)* screenShakeIntensity, 0.0f);

	//while (animationTime2 < 4.0) {
		word->render();
	//}
	glow->render();
...
float PlatformerGame::mapValue(float value, float srcMin, float srcMax, float dstMin, float dstMax) {
...
float PlatformerGame::easeIn(float from, float to, float time) {
